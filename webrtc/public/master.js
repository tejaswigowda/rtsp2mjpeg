...
async function startMaster(localView, remoteView, formValues, onStatsReport, onRemoteDataMessage) {
    ...
    const resolution = formValues.widescreen ? { facingMode: 'environment', width: { ideal: 1280 }, height: { ideal: 720 } } : { width: { ideal: 640 }, height: { ideal: 480 } };
    const constraints = {
    ...
    if (formValues.sendVideo || formValues.sendAudio) {
        try {
            master.localStream = await canvas.captureStream(30);//navigator.mediaDevices.getUserMedia(constraints);
